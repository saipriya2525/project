...
from .module_util import (
    ...
    PreNorm, Residual)
from dino_vit.visualize_attention import outputs_attentions
...
class ConditionalUNet(nn.Module):
    ...
    def forward(self, xt, cond, time):
        ...
        dino_x = outputs_attentions(xt)

        # print("xt = ", xt.shape)
        # print("cond = ", cond.shape)
        # print("x = ", x.shape)
        # print("dino_x = ", dino_x.shape)

        new_dino_x = torch.zeros(xt.shape[0], 1, xt.shape[2], xt.shape[3])
        dino_x = torch.from_numpy(dino_x)
        new_dino_x[:, 0, :, :] = dino_x
        dino_x = new_dino_x.to('cuda:0')
        x = x.to('cuda:0')

        x = torch.cat([x, dino_x], dim=1)
